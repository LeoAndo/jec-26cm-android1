...
def require(condition, detail):
    if not condition:
        raise ValueError(f'教材の整合確認に失敗: {detail}')
...
def assemble(destination, stage):
    ...
    for relative, expected in manifest.items():
        require(hashlib.sha256((STARTER / relative).read_bytes()).hexdigest() == expected, relative)
    ...
    for step in STEPS:
        ...
        for edit in step['edits']:
            ...
            text = file.read_text()
            require(text.count(edit['before']) == 1, (step['id'], edit['file']))
        ...
        for file in checkpoint.glob('*.txt'):
            ...
            require(len(matches) == 1, file)
            require(matches[0].read_bytes() == file.read_bytes(), (step['id'], file.name))
